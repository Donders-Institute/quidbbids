function [T1, PD, R1] = dictmatching(MP2RAGE, INV1, INV2, B1map, varargin)
% function that computes T1 and PD maps using a dictionary matching approach
% inputs:
...
for B1 = B1vector
    ...
    if toc(last_update) > 0.1  % Update every 100ms
        fprintf('\b\b\b\b\b\b\b\b\b\b\b[%9.6f]', B1)
        last_update = tic;
    end
    ...
    if ProbabilityEstimate == 0
        ...
        weight = zeros(interpPt+1, 1);       % initialize interpolation points
        for q = ind_B1'
