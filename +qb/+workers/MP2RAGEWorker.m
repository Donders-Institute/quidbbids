classdef MP2RAGEWorker < qb.workers.Worker
    ...
    methods
        ...
        function get_work_done(obj, workitem)
            ...
            for run = obj.query_ses(obj.BIDS, 'runs', anat{:}, 'part','mag', 'echo',1:999)
                ...
                UNIT1 = obj.query_ses(obj.BIDS, 'data', anat{:}, 'suffix','UNIT1', 'run',char(run));
                INV1  = obj.query_ses(obj.BIDS, 'data', anat{:}, 'suffix','INV1',  'run',char(run));
                INV2  = obj.query_ses(obj.BIDS, 'data', anat{:}, 'suffix','INV2',  'run',char(run));
                ...
                MP2RAGE = obj.getMP2RAGE(INV1, INV2, obj.config.MP2RAGEWorker.InvEff, obj.config.MP2RAGEWorker.EchoSpacing, obj.config.MP2RAGEWorker.NrSegments);

                % Realign & reslice the B1 reference image to the INV2 image
                Vin   = spm_vol(char(B1anat));
                x     = spm_coreg(INV2hdr, Vin, struct('cost_fun', 'mni'));
    ...
    methods (Access = private)
        ...
        function MP2RAGE = getMP2RAGE(INV1, INV2, InvEff, EchoSpacing, NZslices)
