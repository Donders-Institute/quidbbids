function signal = estimateMPRAGE(nimages, MPRAGE_tr, InvTimes, nZslices, FLASH_tr, flipangle, sequence, T1s, InvEff)

% inversion efficiency of the Siemens MP2RAGE PULSE (ideally InvEff=1)
if nargin < 9 || isempty(InvEff)
    InvEff = 0.96;
end
...
if normalsequence
    ...
    TD(1)   = InvTimes(1) - TA_bef;
    E_TD(1) = exp(-TD(1) ./ T1s);
    TD(nimages+1)   = MPRAGE_tr - InvTimes(nimages) - TA_aft;
    E_TD(nimages+1) = exp(-TD(nimages+1) ./ T1s);
    if nimages>1
        for k = 2:(nimages)
            TD(k)   = InvTimes(k) - InvTimes(k-1) - TA;
            E_TD(k) = exp(-TD(k)./T1s);
        end
    end
    for k = 1:(nimages)
...
if waterexcitation
    ...
    E_2A    = exp(-pulseSpace ./ 0.06);     % 60 ms is an estimation of the T2star.. not very relevant
    E_1B    = exp(-(FLASH_tr-pulseSpace) ./ T1s);
    TA      = nZslices * FLASH_tr;
    TA_bef  = nZ_bef * FLASH_tr;
    TD(1)   = InvTimes(1) - TA_bef;
    E_TD(1) = exp(-TD(1) ./ T1s);
    TD(nimages+1) = MPRAGE_tr - InvTimes(nimages) - T_aft;
    E_TD(nimages+1) = exp(-TD(nimages+1) ./ T1s);
...
MZsteadystate          = 1 ./ (1 + InvEff*(prod(cosalfaE1))^nZslices .* prod(E_TD));
MZsteadystatenumerator = M0 * (1 - E_TD(1));
for k = 1:nimages
    % term relative to the image acquisition;
    MZsteadystatenumerator = MZsteadystatenumerator * cosalfaE1(k)^nZslices + M0.*(1-E_1).*(1-cosalfaE1(k)^nZslices) ./ (1-cosalfaE1(k));
...
%% signal
m = 1;
temp = (-InvEff*MZsteadystate*E_TD(1) + M0.*(1-E_TD(1))) .* cosalfaE1(m).^nZ_bef + M0.*(1-E_1) .* (1-cosalfaE1(m)^nZ_bef) ./ (1-cosalfaE1(m));
signal(1) = sinalfa(m)*temp;

if nimages > 1
    for m = 2:nimages
        temp = temp * cosalfaE1(m-1)^nZ_aft + M0.*(1-E_1) .* (1-cosalfaE1(m-1)^nZ_aft) ./ (1-cosalfaE1(m-1));
        temp = (temp * E_TD(m) + M0*(1-E_TD(m))) .* cosalfaE1(m)^nZ_bef + M0.*(1-E_1) .* (1-cosalfaE1(m)^nZ_bef) ./ (1-cosalfaE1(m));
